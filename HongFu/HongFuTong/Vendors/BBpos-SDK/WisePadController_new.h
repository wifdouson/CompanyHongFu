...
typedef enum {
    WisePadCheckCardResult_NoCard,          //CheckCard timeout and no card detected    //到刷卡超時都沒有刷卡或插卡
    WisePadCheckCardResult_InsertedCard,    //Inserted a valid IC card                  //插了卡
    WisePadCheckCardResult_NotIccCard,      //Inserted a Invalid IC card                //插了卡但不是有效IC卡
    WisePadCheckCardResult_BadSwipe,        //Bad swiped                                //刷卡不良好
    WisePadCheckCardResult_SwipedCard,      //Card swiped successfully, card data in cardDataDict //刷了卡，卡數據在cardDataDict字典裡
    WisePadCheckCardResult_MagHeadFail,     //Magnetic stripe reader damage             //刷卡磁頭壞了
    WisePadCheckCardResult_NoResponse,      //For very old firmware only, can ignore it //只用於非常舊的固件，可以不理
    WisePadCheckCardResult_OnlyTrack2,      //For very old firmware only, can ignore it //只用於非常舊的固件，可以不理
    WisePadCheckCardResult_TapCardDetected  //Tapped card                               //拍了卡，需要支持拍卡功能的刷卡器  //Added in 2.10.0
...
typedef enum {
    ...
    WisePadDisplayText_USE_MAG_STRIPE,      //不能讀取卡晶片資料，可能卡的晶片有損壞，請使用磁帶刷卡
...
- (BOOL)isDevicePresent;//is connected?
